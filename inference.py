...
def main():
    ...
    tasks = ["finguard_basic", "finguard_compliance", "finguard_adversarial"]
    
    for current_task in tasks:
        observation = env.reset(task_id=current_task)
        logger.info(f"Environment reset for task: {current_task}")
        ...
        # --- Score Normalization for Validator ---
        # Validator requires score strictly in (0, 1)
        # raw_score is in [-N, N] where N = len(action_log)
        if len(action_log) > 0:
            avg_score = env.score / len(action_log) # range [-1, 1]
            # Map [-1, 1] -> [0, 1]
            zero_to_one = (avg_score + 1) / 2
            # Map [0, 1] -> [0.1, 0.9] to be strictly BETWEEN 0 and 1
            final_parsed_score = round((zero_to_one * 0.8) + 0.1, 3)
        else:
            final_parsed_score = 0.1 # Fallback for no steps

        # Structured output for validator
        print(f"[END] task={current_task} score={final_parsed_score} steps={len(action_log)}", flush=True)

        # Final Summary for logs
        print(f"--- Summary for {current_task} ---")
        print(f"Final Score: {round(env.score, 2)} (Normalized: {final_parsed_score})")
